...
object OptimizedTrafficRestriction {
  ...
  def select(kv2: String): Boolean = {
    // val s = "{\"SystemCpuLoad\":\"0.0\"}"
    val jsonObject = new JsonParser().parse(kv2).getAsJsonObject
    val systemCpuLoad = jsonObject.get("SystemCpuLoad")
      .getAsDouble
    println(systemCpuLoad)
    if (systemCpuLoad > 0.0030) {
      return true
    }
    return false
      ...
      rdd.foreachPartition { iter =>
        ...
        try {
          ...
          iter.map(record => (record.key(), record.value()))
            .filter(kv => select(kv._2))
            .foreach(kv => {
              //发送邮件提醒
              MailUtil.sendMail("1255461002@qq.com", "超出范围" + kv._2)
              //  println(kv._2)
            })
